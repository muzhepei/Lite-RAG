...
环境变量（均为可选，除密码外有合理默认值）：
  ...
  ES_REQUEST_TIMEOUT  HTTP 请求超时（秒），默认 120；低内存 ECS 建索引建议保持 ≥120
"""
...
def get_es() -> Elasticsearch:
    ...
    # 低内存 ECS 上 ES 在内存压力下 GC 较慢；bulk / create 默认 10s 易超时
    raw_timeout = os.environ.get("ES_REQUEST_TIMEOUT", "120").strip()
    try:
        kwargs["request_timeout"] = float(raw_timeout)
    except ValueError:
